...
const LESSONS = [
  {
    branch: "lessons/00-setup",
    parent: "main",
    slug: "00-setup",
    title: "Opening and Setup",
    commitMessage: "lesson(00): add opening and setup",
    content: `# Lesson 00: Opening and Setup
...
`,
  ...
  {
    branch: "lessons/01-intro",
    parent: "lessons/00-setup",
    slug: "01-intro",
    title: "Intro to AI Agents",
    commitMessage: "lesson(01): add intro to AI agents",
    content: `# Lesson 01: Intro to AI Agents
...
`,
  ...
  {
    branch: "lessons/02-getting-started",
    parent: "lessons/01-intro",
    slug: "02-getting-started",
    title: "Getting Started",
    commitMessage: "lesson(02): add getting started",
    content: `# Lesson 02: Getting Started
...
`,
  ...
  {
    branch: "lessons/03-going-deeper",
    parent: "lessons/02-getting-started",
    slug: "03-going-deeper",
    title: "Going Deeper",
    commitMessage: "lesson(03): add going deeper",
    content: `# Lesson 03: Going Deeper
...
`,
  ...
  {
    branch: "lessons/04-tying-it-together",
    parent: "lessons/03-going-deeper",
    slug: "04-tying-it-together",
    title: "Tying It All Together",
    commitMessage: "lesson(04): add tying it all together",
    content: `# Lesson 04: Tying It All Together
...
async function branchExists(branch: string): Promise<boolean> {
  const result = await $`git rev-parse --verify --quiet refs/heads/${branch}`.nothrow().quiet();
...
async function main() {
  console.log("Patterson AI Pets Workshop — Branch Creator");
  console.log("============================================\n");

  const clean = await isWorkingTreeClean();
  if (!clean) {
    console.error("Error: working tree is dirty. Commit or stash changes before running this script.");
    process.exit(1);
  }
  ...
  for (const lesson of LESSONS) {
    const exists = await branchExists(lesson.branch);
    if (exists) {
      console.log(`[skip] ${lesson.branch} already exists`);
      skipped++;
      continue;
    }

    console.log(`[create] ${lesson.branch} from ${lesson.parent}`);

    await $`git checkout ${lesson.parent}`.quiet();
    await $`git checkout -b ${lesson.branch}`.quiet();

    const lessonDir = `lessons/${lesson.slug}`;
    await $`mkdir -p ${lessonDir}`.quiet();

    await Bun.write(`${lessonDir}/LESSON.md`, lesson.content);

    await $`git add ${lessonDir}/LESSON.md`.quiet();
    await $`git commit -m ${lesson.commitMessage}`.quiet();
  ...
  console.log(`\nDone. Created: ${created}, Skipped: ${skipped}, Total: ${LESSONS.length}`);
}
